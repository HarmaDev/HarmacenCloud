...
//  var range = sheet.getDataRange().getValues();
   ...
   try {
     ...
     MailApp.sendEmail(e.parameter.correo, emailSubject, emailContent);
       ...
       try{
         ...
         MailApp.sendEmail("harmacencloud@gmail.com", "Nueva recogida material", "Material a recoger: "+ '\n'+ datos);
  ...
  //       sheet.getRange(range.length+1,1, nuevaRecogida.length, nuevaRecogida[0].length).setValues(nuevaRecogida);
         ...
         MailApp.sendEmail("harmacencloud@gmail.com", "Error report: ", "Ha habido un error en el formulario de recogidas. Mensaje:"+'\n'+e.message);
       }
   } 
   catch (f2) {
